...
if len(SIG)>0:
    ...
    plot(a[,1],a[,2],pch=20,type='b',col='royalblue',xlab='Rank of center within a given window',ylab='Number of significant genes within a given window', xlim=c(0,total),ylim=c(0,max(a[,2])) ,main='ANGEL')
    par(new=T)
    ...
    write.table(file=paste0(input_total,'.left_gene'),left_gene,sep='   ',quote=F,row.names=F,col.names=F)
    write.table(file=paste0(input_total,'.right_gene'),right_gene,sep=' ',quote=F,row.names=F,col.names=F)
